...
const token = reactLocalStorage.get("token");
  ...
  useEffect(() => {
    let getUsersData = async () => {
      await axios
        .get(`http://localhost:5000/service-provider/chat/`,{
          headers: {
            token: token,
          },
        })
  ...
  const searchUser = async (name) => {
    await axios
      .get(`http://localhost:5000/service-provider/chat/${name}`,{
          headers: {
            token: token,
          }
        })
        ...
        <div className="col-5 col-md-4 chat">
          <div className="card chat_card mb-sm-3 mb-md-0 contacts_card">
            ...
            <div className="card-body card_body contacts_body">
              ...
              {users.map((user) => (
              <Link  style={{ textDecoration: 'none' }}  to={`/service-provider/chat/${user.id}`} > 
                <li 
                 className={`${
