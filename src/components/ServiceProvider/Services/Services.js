...
function Services({ match, location }) {
  ...
  useEffect(() => {
    ...
    let getServicesData = async () => {
      await axios
        .get(`http://localhost:5000/service-provider/services/`,{
          headers: {
            token: token,
          } 
        })
  ...
  const deleteService = async (id) => {
    await axios
      .delete(`http://localhost:5000/service-provider/services/${id}`,{
        headers: {
          token: token,
        },
      })
  ...
  const searchService = async (name) => {
    setLoading(true);
    await axios
      .get(`http://localhost:5000/service-provider/services/${name}`, {
        headers: {
          token: token,
        },
      })
