...
function HiredUsers({ match, location }) {
  ...
  useEffect(() => {
    ...
    let getUsersData = async () => {
      await axios
        .get(`http://localhost:5000/service-provider/services/hired-services/${id}`, {
          headers: {
            token: token,
          }, 
        })
  ...
  const paginate = (pageNumber) => setCurrentPage(pageNumber);
   ...
   const deleteEnrollment = async (user_id) => {
    await axios
      .delete(`http://localhost:5000/service-provider/services/enrollment/service/${id}/user/${user_id}`,
      {
        headers: {
          token: token,
        },
