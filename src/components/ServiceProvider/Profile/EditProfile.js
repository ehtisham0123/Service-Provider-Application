...
function EditProfile() {
  ...
  useEffect(() => {
    ...
    let getUserData = async () => {
      await axios
        .get(`http://localhost:5000/service-provider/edit/`,{
          headers: {
            token: token,
          }
        })
        .then((response) => {
          if (response.data) {
            ...
            setOldEmail(response.data.result[0].email);
            setLoading(false);
          }
        })
        .catch((error) => {
 ...
 const handlePhoto = (e) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formdata.country === "null") {
      ...
      await axios.put(`http://localhost:5000/service-provider/update/`, fd, {
          headers: {
            token: token,
      ...
      <form onSubmit={handleSubmit} className="needs-validation">
        ...
        <div className="row">
          ...
          <div className="form-group col-md-12">
            <div
              className="form-control input btn btn-outline-danger"
              data-toggle="modal"
              data-target="#location"
            >
              Change Service Provider Location
            </div>
          </div>
        </div>
