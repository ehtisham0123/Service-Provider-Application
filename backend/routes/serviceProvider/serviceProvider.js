   ...
   var sql = 'SELECT * FROM service_providers WHERE email = ? ';
		...
		if (user) {
			...
			if (!passwordIsValid) {
				...
				var token = jwt.sign({ id: user.id,email:user.email,role:'service_provider' }, 'Ehtisham');
				res.status(201).json({'user_id':user.id, 'user_email':user.email,'user_role':'service_provider','token':token});
			}
		}else{
			res.status(201).json({'error':'invalid credentials'});
		}
            ...
            await db.query(sql, [values], function (err, result) {
              if (err) {
                res.status(201).json({ error: "Error while inseting data" });
                 ...
                 var token = jwt.sign({ id: result.insertId,email:email,role:'service_provider' }, 'Ehtisham');
				         res.status(201).json({'user_id':result.insertId,'user_role':'service_provider','token':token});             
              }
            });
          })
        );
